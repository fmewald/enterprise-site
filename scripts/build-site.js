...
let assetManifest = {};
function createFingerprintAssets(){
  const manifest={};
  for(const [publicPath,srcRel] of Object.entries(FINGERPRINT_SOURCES)){
    const buf=fs.readFileSync(path.join(root,srcRel));
    const hash=hashBuffer(buf).slice(0,8);
    const parsed=path.posix.parse(publicPath);
    const fingerprinted=path.posix.join(parsed.dir,`${parsed.name}.${hash}${parsed.ext}`);
    manifest[publicPath]=fingerprinted;
    write(fingerprinted.replace(/^\//,''),buf);
  }
  return manifest;
}
function applyAssetManifest(html){
  let out=String(html);
  for(const [source,target] of Object.entries(assetManifest)) out=out.split(source).join(target);
  return out;
}
function rewritePublishedHtml(){
  for(const rel of listFiles(dist).filter(x=>x.endsWith('.html'))){
    const file=path.join(dist,rel);
    fs.writeFileSync(file,applyAssetManifest(fs.readFileSync(file,'utf8')));
  }
}
...
function blogCard(a,{featured=false}={}){
  const meta=`<div class="post-card__meta">Atualizado em ${formatDate(a.meta.atualizado||a.meta.data)} · ${escapeHtml(a.meta.autor)}</div>`;
  const tag=`<span class="post-card__cat">${escapeHtml(a.meta.categoria)}</span>`;
  if(featured) return `<article class="post-card post-card--featured"><div class="post-card__visual" aria-hidden="true"><span>Artigo em destaque</span></div><div class="post-card__body">${tag}<h2><a href="/blog/${escapeHtml(a.slug)}/">${escapeHtml(a.meta.titulo)}</a></h2><p>${escapeHtml(a.meta.resumo)}</p>${meta}<a class="post-card__link" href="/blog/${escapeHtml(a.slug)}/">Ler artigo <span aria-hidden="true">→</span></a></div></article>`;
  return `<article class="post-card">${tag}<h2><a href="/blog/${escapeHtml(a.slug)}/">${escapeHtml(a.meta.titulo)}</a></h2><p>${escapeHtml(a.meta.resumo)}</p>${meta}<a class="post-card__link" href="/blog/${escapeHtml(a.slug)}/">Ler artigo <span aria-hidden="true">→</span></a></article>`;
}
function renderBlog(articles){
  let html=read('templates/blog.template.html');
  const [featured,...rest]=articles;
  const feature=featured?blogCard(featured,{featured:true}):'<p>Nenhum artigo publicado no momento.</p>';
  const cards=rest.map(a=>blogCard(a)).join('\n');
  ...
  html=injectFooter(html); html=updateJsonLd(html,false,true); return html;
}
...
function generateRedirects(articles){ const lines=[]; lines.push('https://www.enterprisecontabilidade.com.br/* https://enterprisecontabilidade.com.br/:splat 301!'); for(const [from,to] of Object.entries(config.legacyPages))lines.push(`${from} ${to} 301!`); for(const a of articles)lines.push(`/post.html p=${a.slug} /blog/${a.slug}/ 301!`); for(const p of config.publicPages)if(p.path!=='/')lines.push(`${p.path} /${p.file} 200`); for(const a of articles)for(const old of a.meta.redirect_from)lines.push(`${old} /blog/${a.slug}/ 301!`); return lines.join('\n')+'\n'; }
function validateDist(articles){ const forbiddenNames=/^(tests|scripts|templates|data|config|node_modules|fixtures)(\/|$)/; const forbiddenExt=/\.(md|log|py|toml|lock)$/i; const forbiddenWords=/(AUDITORIA|RELATORIO|CHECKLIST|PENDENCIAS|ROADMAP|CHANGELOG|MAPA-DE-)/i; for(const rel of listFiles(dist)){ if(forbiddenNames.test(rel)||forbiddenExt.test(rel)||forbiddenWords.test(path.basename(rel))||['package.json','package-lock.json','netlify.toml'].includes(rel)) fail(`Arquivo interno em dist: ${rel}`); }
  ...
  if(fs.existsSync(path.join(dist,'post.html'))) fail('post.html legado não deve ser publicado');
  if(fs.existsSync(path.join(dist,'assets/css/styles.css'))||fs.existsSync(path.join(dist,'assets/js/main.js'))) fail('Assets sem fingerprint não devem ser publicados');
  for(const rel of listFiles(dist).filter(x=>x.endsWith('.html'))){ const html=fs.readFileSync(path.join(dist,rel),'utf8'); if(html.includes('/assets/css/styles.css')||html.includes('/assets/js/main.js')) fail(`HTML referencia asset sem fingerprint: ${rel}`); if(rel!=='admin/index.html'){ for(const ref of Object.values(assetManifest)) if(!html.includes(ref)) fail(`HTML sem referência fingerprinted esperada: ${rel} -> ${ref}`); } }
...
function main(){
  const pythonVersion=validatePythonRuntime(); validateAuthority();
  fs.rmSync(dist,{recursive:true,force:true}); fs.mkdirSync(dist,{recursive:true});
  copyDir('assets');
  fs.rmSync(path.join(dist,'assets/css/styles.css'),{force:true});
  fs.rmSync(path.join(dist,'assets/js/main.js'),{force:true});
  assetManifest=createFingerprintAssets();
  copyDir('admin');
  const articles=prepareRenderedArticles(loadArticles()); const blogTemplate=read('templates/blog.template.html'); const common=extractCommon(blogTemplate);
  ...
  write('blog.html',renderBlog(articles));
  for(const a of articles)write(`blog/${a.slug}/index.html`,renderArticle(a,common,articles));
  write('404.html',render404(common)); write('feed.xml',generateFeed(articles)); write('sitemap.xml',generateSitemap(articles));
  write('robots.txt',publicIndexing?`User-agent: *\nAllow: /\nDisallow: /admin/\n\nSitemap: ${siteUrl}/sitemap.xml\n`:'User-agent: *\nDisallow: /\n');
  write('_headers',generateHeaders()); write('_redirects',generateRedirects(articles));
  rewritePublishedHtml();
  validateDist(articles);
  console.log(`Build concluído em dist: ${articles.length} artigos; BUILD_DATE=${buildDate}; TIME_ZONE=${BUILD_TIME_ZONE}; PYTHON=${pythonVersion}; CSS=${assetManifest['/assets/css/styles.css']}; JS=${assetManifest['/assets/js/main.js']}; PUBLIC_INDEXING=${publicIndexing}; CONTEXT=${context}; hash=${dirHash(dist).slice(0,12)}`);
}
main();
